...
/**
 * TotpModal — 6-digit TOTP modal used ONLY on the Login page.
 * This is the login-step MFA verification (Google Authenticator code).
 * For post-login PIN challenges, use PinModal instead.
 */
...
export function TotpModal({
  ...
  const handleChange = (index: number, value: string) => {
    ...
    if (value.length > 1) {
      ...
      pasted.split("").forEach((d, i) => { if (index + i < 6) updated[index + i] = d; });
      setDigits(updated);
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {!isResetMode ? (
              <>
                <div className="flex justify-center gap-2">
                  {digits.map((digit, i) => (
                    <input key={i} ref={(el) => { inputRefs.current[i] = el; }}
                      ...
                      className={`w-10 h-12 text-center text-xl font-mono font-bold rounded-lg border-2 bg-secondary text-foreground focus:outline-none transition-all ${digit ? "border-primary" : "border-border"} focus:border-primary focus:ring-2 focus:ring-primary/20`}
                    />
                  ))}
                </div>
                ...
                {onRequestReset && (
                  <button type="button" onClick={() => setIsResetMode(true)} className="w-full text-xs text-primary hover:underline">
                    Lost Authenticator? Request Reset
                  </button>
                )}
                <p className="text-xs text-center text-muted-foreground mt-4">Codes refresh every 30 seconds</p>
              </>
            ) : (
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <p className="text-sm text-muted-foreground text-center">
                  Please provide a reason for your MFA reset request. Our administrators will review it.
                </p>
                <textarea value={resetMessage} onChange={(e) => setResetMessage(e.target.value)}
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => setIsResetMode(false)}
                    className="flex-1 py-3 bg-secondary text-foreground font-semibold rounded-md hover:bg-secondary/80 transition-all text-sm">Cancel</button>
                  <button type="submit" disabled={!resetMessage.trim() || loading}
                    className="flex-1 py-3 bg-primary text-primary-foreground font-semibold rounded-md hover:bg-primary/90 transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-sm">
